#!/bin/bash
# Auto-deploy script for mainyuk.my.id
...
BRANCH="${1:-main}"
LOG_FILE="/var/www/mainyuk.my.id/deploy/deploy.log"
...
notify_slack() {
  ...
  local slack_url
  slack_url=$(grep SLACK_WEBHOOK_URL /var/www/mainyuk.my.id/deploy/.env | cut -d= -f2-)
...
if ! git pull origin "$BRANCH" >>"$LOG_FILE" 2>&1; then
  ...
  notify_slack "danger" "❌ *mainyuk.my.id* deploy FAILED at \`$COMMIT\`\ngit pull error. Check deploy.log"
  exit 1
fi
...
if git diff --name-only "$COMMIT" "$NEW_COMMIT" 2>/dev/null | grep -q "package-lock.json"; then
  ...
  npm install >>"$LOG_FILE" 2>&1 || {
    notify_slack "danger" "❌ *mainyuk.my.id* deploy FAILED at \`$NEW_COMMIT\`\nnpm install error"
    exit 1
  }
fi
...
pm2 restart mainyuk.my.id >>"$LOG_FILE" 2>&1 || {
  notify_slack "danger" "❌ *mainyuk.my.id* deploy FAILED at \`$NEW_COMMIT\`\npm2 restart error"
  exit 1
}
...
notify_slack "good" "✅ *mainyuk.my.id* deployed successfully\nCommit: \`$NEW_COMMIT\` (branch \`$BRANCH\`)\nBuild time: ${DURATION}s"
